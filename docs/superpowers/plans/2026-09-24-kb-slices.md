...
## Slice 66: Faults found by different rules come back together
...
## Slice 73: An item is checked against its own type
...
- Unknown: Is an item checked against its item's type through the same composed schema as the artifact holding it?
- Needs: none
- Status: planned
...
- Needs: slice 66's rules run over content of any shape (its last three rows)
- Status: planned
...
- 2026-09-26 QUESTION FOR THE SPEC entries each new slice answers, named by the entry that asked it:
  ...
  - 66: kb's own rules run alongside JSON Schema, all errors found (batch 3's question on slices 1.13 and 1.20).
  ...
  - 73: each item is checked against its item's type (batch 4).
