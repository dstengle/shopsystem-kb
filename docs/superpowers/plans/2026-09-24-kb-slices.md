...
- Kind: enabling
- Check: `for i in $(seq 10); do make test 2>&1 | tail -1; done` -> ten lines, each `118 passed`, none with `failed`
...
- Kind: enabling
- Check: `make test` -> `118 passed`; and outside `servicer.py`, `values.py`, `client.py` and `cli.py`, nothing under `src/kb/` reads a request message or its fields (`grep -nE "request\.|\.WhichOneof\(|kb_pb2\.[A-Za-z]+(Request|Operation|Creation|Replacement|Addition|Removal)\b"` over the other modules -> no lines); the journal and the store's commit take the actor and the message as values
...
- Kind: enabling
- Check: `make test` -> `118 passed`; `wc -l src/kb/write.py` -> under 250; `servicer.py` holds no draft, applies no operation, writes no journal entry and makes no commit (`grep -nE "Draft|journal\.|\.commit\(|\.save\(|\.remove\(" src/kb/servicer.py` -> no lines); applying a set, serialising it, and writing and committing it are three functions, none of them doing another's part
...
- Kind: enabling
- Check: `make test` -> `118 passed`; from the first file a set writes to its commit, nothing calls anything that reads a stored file (an AST scan of the write phase's function lists no call to load, holds, ids, artifacts, read_text or read_bytes); a removal's recorded version comes from the draft and a written file's fingerprint from the text written
...
- Kind: enabling
- Check: `make test` -> `118 passed`; `grep -c kb_pb2 src/kb/write.py` -> `0`; the write pipeline answers with what each operation did, and `servicer.py` makes the response from it
...
- 2026-09-26 QUESTION FOR THE SPEC: is a set checked as a whole after every operation is applied (CLAUDE.md rule 4), so that a set changing a type refuses an artifact of that type made earlier in it, or is each operation checked against the store as the operations before it left it (the Write path as built and the `Apply` row today)?
  Next: writing-plans over slices 53 to 62, one task per slice, in `2026-09-26-kb-refactor-implementation.md`.
